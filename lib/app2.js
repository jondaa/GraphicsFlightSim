...
function main(){
	...
	if(gl){
		...
		skyboxTbtx.txNum = 2;
		skyboxTbtx.onload = terrainLoaderCallback;
...
//		perspectiveMatrix = utils.MakePerspective(60, w/h, 0.1, 1000.0);
			...
			for(var i = 0; i < treeFieldSize; i+=1){
				for(j = 0; j < treeFieldSize; j+=1){
					treeLocationx[i*treeFieldSize+j]=Math.round(200*Math.random()+1);
					treeLocationy[i*treeFieldSize+j]=Math.round(200*Math.random()+1);
...
function drawScene() {
    ...
    currentPos= rotationDirY.mul(rotationDirX.mul(rotationDirZ));
		...
		worldMatrix = utils.multiplyMatrices(dvecmat, utils.MakeScaleMatrix(0.5));
		xaxis = [dvecmat[0],dvecmat[4],dvecmat[8]];
		yaxis = [dvecmat[1],dvecmat[5],dvecmat[9]];
		zaxis = [dvecmat[2],dvecmat[6],dvecmat[10]];


		cx = carX+10*Math.cos(-angleY*2-Math.PI/2);
		cy = carY+(pitch+15)*0.3;
		cz = carZ+10*Math.sin(-angleY*2-Math.PI/2);
		// car motion
...
var heightAtXZ = function (x,z){
	...
	//if(x==x_ && z==z_)
		return heightdata[x_*256+z_]*5;


	return (1-z+z_)*(1-x+x_)*heightAtXZ(x_,z_)+(1-z+z_)*(x-x_)*heightAtXZ(x_+1,z_)+
		(z-z_)*(1-x+x_)*heightAtXZ(x_,z_+1)+(z-z_)*(1-x+x_)*heightAtXZ(x_+1,z_+1);
