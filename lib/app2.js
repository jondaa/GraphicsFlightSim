...
function isThereATreeHere(){
	...
	for (i=0; i<treeFieldSize*treeFieldSize; i++){
		console.log(Math.sqrt(Math.pow(x-treeLocation.x[i],2) + Math.pow(z-treeLocation.y[i],2)));
		if(Math.sqrt(Math.pow(x-treeLocation.x[i],2) + Math.pow(z-treeLocation.y[i],2))<5){
			if( y<heightAtXZ(x,z)+TREEHEIGHT){
				crashSound.play()
