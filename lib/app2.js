...
function drawScene() {
    ...
    gl.uniform1f(program.materialSpecPower, 100);
		...
		normalMatrix1 = utils.identityMatrix();
		...
		gl.uniformMatrix4fv(program.normalMatrix, gl.FALSE, normalMatrix1);
		for (i=0; i < 6; i++){
		...
		for(i=0; i<NUM_RINGS; i++){
			...
			gl.uniformMatrix4fv(program.vertexMatrix, gl.FALSE, utils.transposeMatrix(worldMatrix1));
			gl.uniformMatrix4fv(program.matrix, gl.FALSE, utils.transposeMatrix(projectionMatrix1));
			gl.uniformMatrix4fv(program.normalMatrix, gl.FALSE, utils.invertMatrix(utils.transposeMatrix(normalMatrix1)));
		...
		gl.uniformMatrix4fv(program.normalMatrix, gl.FALSE, utils.invertMatrix(utils.transposeMatrix(worldMatrix)));
...
function haveWeCrashed(){
	...
	var xFound= false;
	for (i=0; i<treeFieldSize*treeFieldSize; i++){
		if(Math.sqrt(Math.pow(x-treeLocation.x[i],2) + Math.pow(z-treeLocation.y[i],2))<2){
			if( y<heightAtXZ(x,z)+TREEHEIGHT &&  y>=heightAtXZ(x,z)){
				crashSound.play()
